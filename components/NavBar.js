...
const routes = [{id:0,name:'جستجو',route:'search'},
  {id:1,name:'اخیر',route:'recent'},
  {id:2,name:'علاقه مندی ها',route:'bookmarks'},
  {id:3,name:'تنظیمات',route:'settings'}];

// svg code for nav item icons, from radix-icons
const svgs = {
  search: (
    <svg
      width="28"
...
export default function NavBar() {
  ...
  // check if currently on /define
  const ifDefine = router.pathname.substring(1) === 'define';
        ...
        {!ifDefine && (
          <ul>
            {routes.map((route,index) => {
              return <NavBarItem route={route} svg={svgs[route.route]} key={route.id} />;
            })}
          </ul>
        )}
        {/* return different navbar if on /define */}
        {ifDefine && (
          <ul>
            <NavBarItem route="Search" href="/search" svg={svgs['back']} />
            <BookmarkCheckBox />
          </ul>
        )}
      </nav>
